...
public abstract class AbstractIncrementalProjectBuilder extends org.eclipse.core.resources.IncrementalProjectBuilder {
	...
	protected abstract IProject[] obtainInterestingProjects(int kind, Map<String,String> args);

	protected abstract boolean buildResourceDelta(IResourceDelta delta,
			int kind, Map<String,String> args, IProgressMonitor monitor);
	...
	public static void addBuilder (final IProject project, final String builderID, final boolean before, final IProgressMonitor monitor) throws CoreException {
		...
		if (before) {
			System.arraycopy(desc.getBuildSpec(), 0, commands, 1, desc.getBuildSpec().length);
			commands[0] = builder;
		}
		else {
			System.arraycopy(desc.getBuildSpec(), 0, commands, 0, desc.getBuildSpec().length);
		...
		for (int i = 0; i<desc.getBuildSpec().length; i++) {
			if (desc.getBuildSpec()[i].getBuilderName().equals(builderID)) {
				...
				if (i < commands.length)
					commands[i] = desc.getBuildSpec()[i];
			}
		}
		if (-1 != pos) {
			desc.setBuildSpec(commands);
